...
kappa_calc <- function(df, var1, var2){
  ...
  pB <- ggplot(df) + 
    geom_venn(aes(A = get(var1), B = get(var2)), fill_color = c("indianred1", "darkslategray2"), set_names = c("", ""), auto_scale = TRUE, show_percentage = FALSE, text_size = 0.0001, stroke_size = 0.1) + 
    coord_fixed() +
    theme_void() +
