...
if (!file.exists(here("output/tables/table2_fup_stats.csv"))) {
  ...
  plot_some_categorical_variables <- function(data_in, stringsearch){
    data_plot <- data_in %>% 
    ...
    ggplot(data_plot, aes(x = survey_response, y = p, label = nice_n, colour = level, fill = level)) +
      ...
      ylim(c(0,100)) +
      labs(y = "%", x = "Survey round", colour = "Response", fill = "Response") +
      theme_ali() +
  ...
  p3a <- plot_some_categorical_variables(tab2stats, catvars[1])
  p3b <- plot_some_categorical_variables(tab2stats, catvars[2])
  p3c <- plot_some_categorical_variables(tab2stats, catvars[3])
  p3da <- plot_some_categorical_variables(tab2stats, catvars[4])
  p3db <- plot_some_categorical_variables(tab2stats, catvars[5])
  p3dc <- plot_some_categorical_variables(tab2stats, catvars[6])
  p3dd <- plot_some_categorical_variables(tab2stats, catvars[7])
  p3de <- plot_some_categorical_variables(tab2stats, catvars[8])
  ...
  ggsave(filename=here::here("output", "plots","openprompt_figure3.tiff"), p3, dpi=450, width = 26, height = 10, units = "in", bg = "white")
  ggsave(filename=here::here("output", "plots","openprompt_figure3.png"),  p3, dpi=450, width = 26, height = 10, units = "in", bg = "white")
}
